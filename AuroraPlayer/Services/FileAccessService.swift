...
class FileAccessService: ObservableObject {
    ...
    // MARK: - Estadísticas de reproducción (3.0)
    // ✅ Tracking 100% local (sin red y sin tocar el caché de biblioteca v15):
    //   · playCounts      → cuántas veces se ha INICIADO cada canción.
    //   · playTimes       → segundos REALMENTE escuchados (tramos, no duración).
    //   · lastPlayedDates → base del shuffle ponderado y de "Descubrimientos".
    // Persisten en `play-stats.json` (Application Support), aparte del caché.
    @Published private(set) var playCounts: [UUID: Int] = [:]
    @Published private(set) var playTimes: [UUID: TimeInterval] = [:]
    @Published private(set) var lastPlayedDates: [UUID: Date] = [:]

    /// ✅ CARGA PEREZOSA: el archivo se lee la primera vez que se necesita (el
    /// arranque no paga I/O extra). Siempre en el hilo principal.
    private var playStatsLoaded = false
    private let playStatsFileName = "play-stats.json"
    private static let playStatsQueue = DispatchQueue(label: "com.aurora.playstats", qos: .utility)
    /// ✅ ROTACIÓN: por encima de este tamaño se descartan las entradas de
    /// canciones que ya no están en la biblioteca (nunca las vigentes).
    private let playStatsMaxBytes = 500 * 1024
    /// ✅ Acumulador EN VIVO (no @Published a propósito: el observador del reloj
    /// llama 2-3 veces por segundo y no debe invalidar la UI en cada tick).
    private var pendingListenSeconds: TimeInterval = 0
    private var pendingListenSongID: UUID?
    private var lastListenTickDate: Date?
    /// ✅ Volcado del acumulador: como máximo una escritura cada 15 s.
    private let listenFlushInterval: TimeInterval = 15

    private var playStatsURL: URL? {
        guard let directory = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first else { return nil }
        return directory.appendingPathComponent(playStatsFileName)
    }

    /// Formato persistido: claves = UUID en texto (JSON estable y compacto).
    private struct PersistedPlayStats: Codable {
        var counts: [String: Int] = [:]
        var times: [String: Double] = [:]
        var lastPlayed: [String: Date] = [:]
    }

    func ensurePlayStatsLoaded() {
        guard !playStatsLoaded else { return }
        playStatsLoaded = true
        guard let url = playStatsURL,
              let data = try? Data(contentsOf: url),
              let decoded = try? JSONDecoder().decode(PersistedPlayStats.self, from: data) else { return }
        playCounts = decoded.counts.reduce(into: [:]) { result, pair in
            if let id = UUID(uuidString: pair.key) { result[id] = pair.value }
        }
        playTimes = decoded.times.reduce(into: [:]) { result, pair in
            if let id = UUID(uuidString: pair.key) { result[id] = pair.value }
        }
        lastPlayedDates = decoded.lastPlayed.reduce(into: [:]) { result, pair in
            if let id = UUID(uuidString: pair.key) { result[id] = pair.value }
        }
        AppLog.info(.library, "Estadísticas de reproducción: \(playCounts.count) canciones con historial")
    }

    // MARK: Lectura (carga perezosa incluida)

    func playCount(for songID: UUID) -> Int {
        ensurePlayStatsLoaded()
        return playCounts[songID] ?? 0
    }

    func playTime(for songID: UUID) -> TimeInterval {
        ensurePlayStatsLoaded()
        return playTimes[songID] ?? 0
    }

    func lastPlayed(for songID: UUID) -> Date? {
        ensurePlayStatsLoaded()
        return lastPlayedDates[songID]
    }

    /// ✅ Peso del SHUFFLE INTELIGENTE: prioriza lo que suena menos, lo que hace
    /// mucho que no suena y lo que te gusta. El azar (0…1) lo añade el motor al
    /// ordenar, para que dos mezclas seguidas no sean idénticas.
    func smartShuffleWeight(for song: Song) -> Double {
        var weight = 3.0
        if playCount(for: song.id) < 3 { weight += 2.0 }
        if let last = lastPlayed(for: song.id) {
            if Date().timeIntervalSince(last) > 7 * 24 * 3600 { weight += 1.5 }
        } else {
            weight += 1.5
        }
        if isLiked(song) { weight += 0.5 }
        return weight
    }

    // MARK: Registro

    /// ✅ Una canción empieza a sonar (lo llama RootTabView vía AudioEngine).
    func recordPlayStarted(songID: UUID) {
        ensurePlayStatsLoaded()
        // Cerrar el tramo de la canción anterior antes de cambiar de contexto.
        flushPendingListeningTime()
        playCounts[songID, default: 0] += 1
        lastPlayedDates[songID] = Date()
        savePlayStats()
    }

    /// ✅ Segundos escuchados acumulados explícitamente (volcado manual).
    func recordPlayProgress(songID: UUID, seconds: TimeInterval) {
        guard seconds > 0 else { return }
        ensurePlayStatsLoaded()
        playTimes[songID, default: 0] += seconds
        savePlayStats()
    }

    /// ✅ Lo llama RootTabView en cada tick del reloj de reproducción
    /// (0,4 s en primer plano / 3 s en segundo plano). Acumula SEGUNDOS DE
    /// ESCUCHA REALES midiendo el reloj de pared entre ticks — no la posición
    /// del audio — de modo que los seeks no inflan el tiempo. Solo escribe en
    /// `playTimes` cada ~15 s, al pausar o al cambiar de canción.
    func accumulateListeningTick(songID: UUID?, isPlaying: Bool) {
        let now = Date()
        defer { lastListenTickDate = isPlaying ? now : nil }
        guard isPlaying, let songID else {
            flushPendingListeningTime()
            return
        }
        if pendingListenSongID != songID {
            flushPendingListeningTime()
            pendingListenSongID = songID
        }
        guard let last = lastListenTickDate else { return }
        let delta = now.timeIntervalSince(last)
        // Ticks normales: 0,4 s / 3 s. Un salto mayor (seek, reanudación tras un
        // parón del run loop) NO es tiempo escuchado y se descarta.
        guard delta > 0, delta <= 5 else { return }
        pendingListenSeconds += delta
        if pendingListenSeconds >= listenFlushInterval {
            flushPendingListeningTime()
        }
    }

    private func flushPendingListeningTime() {
        defer {
            pendingListenSeconds = 0
            pendingListenSongID = nil
        }
        guard pendingListenSeconds > 0, let songID = pendingListenSongID else { return }
        ensurePlayStatsLoaded()
        playTimes[songID, default: 0] += pendingListenSeconds
        savePlayStats()
    }

    private func savePlayStats() {
        prunePlayStatsIfNeeded()
        let payload = PersistedPlayStats(
            counts: Dictionary(uniqueKeysWithValues: playCounts.map { ($0.key.uuidString, $0.value) }),
            times: Dictionary(uniqueKeysWithValues: playTimes.map { ($0.key.uuidString, $0.value) }),
            lastPlayed: Dictionary(uniqueKeysWithValues: lastPlayedDates.map { ($0.key.uuidString, $0.value) })
        )
        guard let url = playStatsURL else { return }
        // ✅ Codificar y escribir FUERA del hilo principal (igual que el log):
        // el snapshot de valores ya se tomó aquí.
        Self.playStatsQueue.async {
    ...
    /// ✅ Rotación por tamaño: si el archivo ya supera los 500 KB, se descartan
    /// las entradas de canciones que NO están en la biblioteca (las vigentes
    /// conservan su historial intacto).
    private func prunePlayStatsIfNeeded() {
        guard !songs.isEmpty, let url = playStatsURL,
              let attributes = try? FileManager.default.attributesOfItem(atPath: url.path),
              let size = attributes[.size] as? Int,
              size > playStatsMaxBytes else { return }
        let liveIDs = Set(songs.map(\.id))
        let before = playCounts.count
        playCounts = playCounts.filter { liveIDs.contains($0.key) }
        playTimes = playTimes.filter { liveIDs.contains($0.key) }
        lastPlayedDates = lastPlayedDates.filter { liveIDs.contains($0.key) }
        AppLog.info(.library, "Estadísticas rotadas: \(before - playCounts.count) canciones fuera de la biblioteca")
    }
